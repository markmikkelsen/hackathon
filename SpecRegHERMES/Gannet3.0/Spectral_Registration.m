...
nlinopts = statset(nlinopts, 'MaxIter', 1e5, 'UseParallel', true, 'Display', 'fial');
...
while(SpecRegLoop>(-1))
    ...
    if(OnWhat) %Read water data
        ...
        if(nargin==3)
            if(Dual==1) || (Dual==2) % ADH
                %This code runs twice, first for ONs, second for OFFs.
                %SpecRegLoop;
                %size(real(MRS_struct.fids.data(:,(SubspecToAlign==SpecRegLoop))));
                flatdata(:,1,:)=real(MRS_struct.fids.data(:,(SubspecToAlign==SpecRegLoop)));
                flatdata(:,2,:)=imag(MRS_struct.fids.data(:,(SubspecToAlign==SpecRegLoop)));
            end
        else
            %First, take the complex data and turn it into a real matrix
        ...
        for corrloop=1:size(flatdata,3)
            ...
            if(nargin==3)
                if(Dual==1)
                    ...
                    averages_per_dynamic=find(SubspecToAlign~=(SubspecToAlign(1)),1)-1;
        ...
        if(SpecRegLoop==0)
            ...
            if(nargin==3)
                if(Dual==1)
                    ...
                    ChoCrMeanSpecON = mean(AllFramesFTrealign(cclb:ccub,(SubspecToAlign==1)),2);
                    ChoCrMeanSpecOFF = mean(AllFramesFTrealign(cclb:ccub,(SubspecToAlign==0)),2);
                    ...
                    AllFramesFTrealign(:,(SubspecToAlign==1))=AllFramesFTrealign(:,(SubspecToAlign==1))*exp(1i*pi/180*(ChoCrMeanSpecFitON(4)));%phase
                    AllFramesFTrealign(:,(SubspecToAlign==0))=AllFramesFTrealign(:,(SubspecToAlign==0))*exp(1i*pi/180*(ChoCrMeanSpecFitOFF(4)));%phase
                    ...
                    AllFramesFTrealign(:,(SubspecToAlign==1))=circshift(AllFramesFTrealign(:,(SubspecToAlign==1)), [-ChoCrFreqShift_pointsON 0]);%freq
                    ...
                    AllFramesFTrealign(:,(SubspecToAlign==0))=circshift(AllFramesFTrealign(:,(SubspecToAlign==0)), [-ChoCrFreqShift_pointsOFF 0]);%freq                    
