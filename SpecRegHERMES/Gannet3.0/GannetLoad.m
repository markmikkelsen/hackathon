...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    DataSize = size(FullData,2);
        ...
        if(strcmpi(MRS_struct.p.Reference_compound,'H2O'))        
            ...
            FrameMax = max(abs(real(AllFramesFT(water_range,:))),[],1);
            FrameMaxPos = zeros(1,size(AllFramesFT,2));
            for kk = 1:size(AllFramesFT,2)
                FrameMaxPos(kk) = find(abs(real(AllFramesFT(:,kk))) == FrameMax(kk));
            end
            ...
            switch MRS_struct.p.AlignTo % MM: use AllFramesFT instead of AllFramesFTrealign
                ...
                case 'SpecRegHERMES'
                    %Dual-channel Spectral Registration is applied separately to ON and OFF and they are coregistered after...
                    [AllFramesFTrealign MRS_struct] = Spectral_Registration2(MRS_struct);                    
                case 'SpecRegQuad'
                    %Dual-channel Spectral Registration is applied separately to ON and OFF and they are coregistered after...
                    [AllFramesFTrealign MRS_struct] = Spectral_Registration(MRS_struct,0,2);
                    [AllFramesFTrealign MRS_struct] = MCMC_Realign(AllFramesFTrealign, MRS_struct);
            end %end of switch for alignment targhelp arrayfunet
            
            MRS_struct.spec.subspec.prealign.expA(ii,:) = mean(AllFramesFT(:,1:4:end),2);
            MRS_struct.spec.subspec.prealign.expB(ii,:) = mean(AllFramesFT(:,2:4:end),2);
            MRS_struct.spec.subspec.prealign.expC(ii,:) = mean(AllFramesFT(:,3:4:end),2);
            MRS_struct.spec.subspec.prealign.expD(ii,:) = mean(AllFramesFT(:,4:4:end),2);
            MRS_struct.spec.subspec.prealign.sum(ii,:) = MRS_struct.spec.subspec.prealign.expA(ii,:) + ...
                                                         MRS_struct.spec.subspec.prealign.expB(ii,:) + ...
                                                         MRS_struct.spec.subspec.prealign.expC(ii,:) + ...
                                                         MRS_struct.spec.subspec.prealign.expD(ii,:);
            
            MRS_struct.spec.subspec.postalign.expA(ii,:) = mean(AllFramesFTrealign(:,1:4:end),2);
            MRS_struct.spec.subspec.postalign.expB(ii,:) = mean(AllFramesFTrealign(:,2:4:end),2);
            MRS_struct.spec.subspec.postalign.expC(ii,:) = mean(AllFramesFTrealign(:,3:4:end),2);
            MRS_struct.spec.subspec.postalign.expD(ii,:) = mean(AllFramesFTrealign(:,4:4:end),2);
            MRS_struct.spec.subspec.postalign.sum(ii,:) = MRS_struct.spec.subspec.postalign.expA(ii,:) + ...
                                                          MRS_struct.spec.subspec.postalign.expB(ii,:) + ...
                                                          MRS_struct.spec.subspec.postalign.expC(ii,:) + ...
                                                          MRS_struct.spec.subspec.postalign.expD(ii,:);
                                                      
            if ~strcmp(gabafile{ii}((end-3):end),'.mat')
                MRS_struct.spec.subspec.prealign.resid(ii,:) = MRS_struct.spec.subspec.prealign.expC(ii,:) - ...
                                                               MRS_struct.spec.subspec.prealign.expB(ii,:) - ...
                                                               MRS_struct.spec.subspec.prealign.expA(ii,:) + ...
                                                               MRS_struct.spec.subspec.prealign.expD(ii,:);
                MRS_struct.spec.subspec.postalign.resid(ii,:) = MRS_struct.spec.subspec.postalign.expC(ii,:) - ...
                                                                MRS_struct.spec.subspec.postalign.expB(ii,:) - ...
                                                                MRS_struct.spec.subspec.postalign.expA(ii,:) + ...
                                                                MRS_struct.spec.subspec.postalign.expD(ii,:);
            else
                MRS_struct.spec.subspec.prealign.resid(ii,:) = MRS_struct.spec.subspec.prealign.expA(ii,:) - ...
                                                               MRS_struct.spec.subspec.prealign.expB(ii,:) - ...
                                                               MRS_struct.spec.subspec.prealign.expC(ii,:) + ...
                                                               MRS_struct.spec.subspec.prealign.expD(ii,:);
                MRS_struct.spec.subspec.postalign.resid(ii,:) = MRS_struct.spec.subspec.postalign.expA(ii,:) - ...
                                                                MRS_struct.spec.subspec.postalign.expB(ii,:) - ...
                                                                MRS_struct.spec.subspec.postalign.expC(ii,:) + ...
                                                                MRS_struct.spec.subspec.postalign.expD(ii,:);
            end
        ...
        for kk = 1:length(reg)
            if MRS_struct.p.HERMES    % MGSaleh & MM 2016: for HERMES of GSH/Lac and GABAGlx/GSH
                ...
                if strcmp(MRS_struct.p.target, 'GABAGlx') && strcmp(MRS_struct.p.target2, 'GSH') && strcmp(MRS_struct.p.AlignTo, 'SpecRegQuad2')                    
