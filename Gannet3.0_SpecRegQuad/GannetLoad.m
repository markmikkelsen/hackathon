    ...
    DataSize = size(FullData,2);
        ...
        if(strcmpi(MRS_struct.p.Reference_compound,'H2O'))        
            ...
            switch MRS_struct.p.AlignTo % MM: use AllFramesFT instead of AllFramesFTrealign
                ...
                case 'SpecRegQuad'
                    MRS_struct.fids.ON_OFF_notbroken=MRS_struct.fids.ON_OFF;
                    %Dual-channel Spectral Registration is applied separately to ON and OFF and they are coregistered after...
                    [AllFramesFTrealign MRS_struct] = Spectral_Registration(MRS_struct,0,2);
                    
                    MRS_struct.spec.ON_ON(ii,:) = mean(AllFramesFTrealign(:,3:4:end),2);
                    MRS_struct.spec.OFF_OFF(ii,:) = mean(AllFramesFTrealign(:,4:4:end),2);
                    MRS_struct.spec.ON_OFF(ii,:) = mean(AllFramesFTrealign(:,2:4:end),2);
                    MRS_struct.spec.OFF_ON(ii,:) = mean(AllFramesFTrealign(:,1:4:end),2);
        ...
        for kk = 1:length(reg)
            if MRS_struct.p.HERMES    % MGSaleh & MM 2016: for HERMES of GSH/Lac and GABAGlx/GSH
                ...
                xx2=mean(AllFramesFTrealign(:,((MRS_struct.fids.ON_OFF_notbroken==0)'&(MRS_struct.out.reject(:,ii)==0))),2);
                yy2=mean(AllFramesFTrealign(:,((MRS_struct.fids.ON_OFF_notbroken==1)'&(MRS_struct.out.reject(:,ii)==0))),2);
